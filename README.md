# [live-web-instrument](https://github.com/lafefspietz/live-web-instrument)

Live web instrument with Python on back end and HTML front end

 - [index.html](index.html)
 - [edit-files.html](edit-files.html)
 - [live-web-instrument.json](live-web-instrument.json)
 - [live-web-instrument.html](live-web-instrument.html)
 - [live_web_instrument.ipynb](live_web_instrument.ipynb)
 - [live_web_instrument.py](live_web_instrument.py)
 - [live-web-instrument.bat](live-web-instrument.bat)
 - [plots.md](plots.md)
 - [data.md](data.md)
 - [bridge.php](bridge.php)
 - [save-file.php](save-file.php)
 - [load-file.php](load-file.php)
